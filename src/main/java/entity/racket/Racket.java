...
public class Racket{
    ...
    //creation de la raquette
    public Racket(Coordinates c, int longueur, int largeur, int speed, int type, Vector direction, boolean fixeY, boolean jump) {
            this.c = c;
    ...
    public boolean CollisionRacket(Coordinates c){
        if(c.getX() > this.c.getX() && c.getX() < this.c.getX() + this.longueur && c.getY() > this.c.getY() && c.getY() < this.c.getY() + this.largeur){
            return true;
        }
        return false;
    }
